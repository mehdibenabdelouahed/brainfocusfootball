...
@section('title', __('pricing.checkout_title'))
...
<div class="min-h-screen bg-slate-950 text-white flex items-center justify-center py-12 px-4">
    <div class="max-w-md w-full">
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold mt-6">{{ __('pricing.checkout_header') }}</h1>
            <p class="text-slate-400 mt-2">{!! __('pricing.checkout_subheader', ['plan' => $plan]) !!}</p>
        </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl">
            <div class="mb-6 pb-6 border-b border-slate-800">
                <div class="flex justify-between items-center">
                    <span class="text-slate-400">{{ __('pricing.total_to_pay') }}</span>
                    <span class="text-2xl font-black text-white">
                        @if($plan === 'STANDARD') 29€
                        @elseif($plan === 'PRO') 79€
                        @elseif($plan === 'ACADEMIE') 199€
                        @elseif($plan === 'PREMIUM') 4,99€
                        @else 0€ @endif
                        <span class="text-sm text-slate-500 font-normal">{{ __('pricing.per_month') }}</span>
                    </span>
                </div>
            </div>
            ...
            <form action="{{ route('checkout.process') }}" method="POST">
                ...
                <div class="space-y-4 mb-6">
                    <div>
                        <label class="block text-sm font-semibold text-slate-300 mb-1">{{ __('pricing.card_name') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-300 mb-1">{{ __('pricing.card_number') }}</label>
                        <div class="relative">
                        ...
                        <p class="text-[10px] text-slate-500 mt-1">{{ __('pricing.card_simulation_hint') }}</p>
                    </div>
                </div>

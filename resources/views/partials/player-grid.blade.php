            ...
            <div class="group bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden hover:border-amber-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-amber-500/10">
                ...
                <div class="p-5">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button @click="$dispatch('toggle-compare', { id: {{ $player->id }}, name: '{{ $player->first_name }} {{ $player->last_name }}', photo: '{{ $player->profile_photo ? asset('storage/' . $player->profile_photo) : '' }}' })"
                                :class="isInCompare({{ $player->id }}) ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'"

            ...
            <div class="group bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden hover:border-amber-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-amber-500/10">
                ...
                <div class="p-5">
                    ...
                    <div class="flex items-center gap-2 text-sm text-slate-400 mb-4">
                        ...
                        <span>{{ $player->current_club ?? __('profile.no_club') }}</span>
                    </div>

                    {{-- Stats rapides --}}
                    <div class="grid grid-cols-3 gap-2 mb-5 py-3 border-y border-slate-800">
                        <div class="text-center">
                            <span class="block text-xs text-slate-500">{{ __('profile.age') }}</span>
                            <span class="font-semibold text-slate-300">
                                {{ $player->date_of_birth ? \Carbon\Carbon::parse($player->date_of_birth)->age : '-' }}
                            </span>
                        </div>
                        <div class="text-center border-l border-slate-800">
                            <span class="block text-xs text-slate-500">{{ __('profile.preferred_foot') }}</span>
                            <span class="font-semibold text-slate-300 font-mono">
                                {{ $player->dominant_foot ? substr($player->dominant_foot === 'Droit' ? __('profile.foot_right') : ($player->dominant_foot === 'Gauche' ? __('profile.foot_left') : __('profile.foot_ambidextrous')), 0, 1) : '-' }}
                            </span>
                        </div>
                        <div class="text-center border-l border-slate-800">
                            <span class="block text-xs text-slate-500">{{ __('profile.height_label') }}</span>
                            <span class="font-semibold text-slate-300">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button @click="$dispatch('toggle-compare', { id: {{ $user->id }}, name: '{{ $player->first_name }} {{ $player->last_name }}', photo: '{{ $player->profile_photo ? asset('storage/' . $player->profile_photo) : '' }}' })"
                                ...
                                title="{{ __('profile.compare_player') }}">
                        ...
                        @auth
                            @if(auth()->user()->isRecruiter() && auth()->user()->recruiter)
                                ...
                                <button 
                                    ...
                                    title="{{ $isFav ? __('profile.remove_favorite') : __('profile.add_favorite') }}">
    ...
    <script>
        function toggleFavorite(btn, playerId) {
            ...
            .then(data => {
                ...
                btn.title = newIsFav ? "{{ __('profile.remove_favorite') }}" : "{{ __('profile.add_favorite') }}";
...
@else
    ...
    <div class="text-center py-20 bg-slate-900/30 rounded-3xl border border-slate-800 border-dashed">
        ...
        <h3 class="text-xl font-bold text-white mb-2">{{ __('profile.no_players_found') }}</h3>
        <p class="text-slate-400 mb-6">{{ __('profile.adjust_filters_desc') }}</p>

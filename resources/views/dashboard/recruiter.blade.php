...
@section('title', __('dashboard.title_recruiter'))
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <span class="inline-block px-3 py-1 rounded-full bg-indigo-500/15 text-indigo-400 text-xs font-bold border border-indigo-500/25 mb-4 uppercase tracking-wider">
                        {{ __('dashboard.role_recruiter') }}
                    </span>
                    <h1 class="text-4xl md:text-5xl font-black tracking-tight mb-3">
                        {!! __('dashboard.welcome', ['name' => '<span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">' . e($user->name) . '</span>']) !!}
                    </h1>
                    <p class="text-slate-400 text-lg max-w-2xl">{{ __('dashboard.recruiter_tagline') }}</p>
                </div>
    ...
    <div class="container mx-auto px-4 max-w-7xl py-10 space-y-10">
        ...
        <div class="bg-slate-900/70 backdrop-blur-md rounded-3xl border border-slate-700/50 p-8 shadow-2xl">
            ...
            <form action="{{ route('talents') }}" method="GET">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">{{ __('dashboard.search_name_club') }}</label>
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('dashboard.search_name_club_placeholder') }}" 
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">{{ __('dashboard.position') }}</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">{{ __('dashboard.age_group') }}</label>
                        <select name="age_group" class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 transition">
                            <option value="">{{ __('dashboard.all_age_groups') }}</option>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">{{ __('dashboard.level') }}</label>
                        <select name="level" class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 transition">
                            <option value="">{{ __('dashboard.all_levels') }}</option>
                ...
                <div class="border-t border-slate-800 pt-6 mb-6">
                    ...
                    @if(in_array(Auth::user()->recruiterPlan(), ['STANDARD', 'PRO', 'ACADEMIE']))
                        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                            @foreach([
                                ...
                                <div>
                                    <label class="block text-xs text-slate-500 mb-2">{{ __('dashboard.radar_min_label', ['label' => __('dashboard.radar_labels.' . $field['label'])]) }}</label>
                                    <div class="relative">
                    ...
                    @else
                        <div class="bg-slate-900 border border-slate-700/50 rounded-xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div>
                                <h3 class="text-white font-bold mb-1">{{ __('dashboard.unlock_radar_search') }}</h3>
                                <p class="text-sm text-slate-400">{{ __('dashboard.unlock_radar_search_desc') }}</p>
                            </div>
        ...
        <div class="bg-slate-900/70 backdrop-blur-md rounded-3xl border border-slate-700/50 p-8 shadow-2xl">
            <div class="flex items-center justify-between mb-6">
                ...
                @if($favorites->count() > 0)
                    <a href="{{ route('talents') }}" class="text-sm text-slate-400 hover:text-white transition">{{ __('dashboard.view_all_talents') }} →</a>
                @endif
            </div>
            ...
            @if($favorites->isEmpty())
                <div class="text-center py-12 border-2 border-dashed border-slate-800 rounded-2xl">
                    ...
                    <p class="text-slate-400 font-medium mb-2">{{ __('dashboard.no_favorites') }}</p>
                    <p class="text-slate-500 text-sm mb-6">{{ __('dashboard.no_favorites_desc') }}</p>
                        ...
                        <div class="bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden hover:border-amber-500/40 transition group">
                            <div class="aspect-[3/2] relative overflow-hidden bg-slate-800">
                                ...
                                <div class="absolute top-2 right-2 bg-amber-500 text-slate-950 text-[10px] font-black px-2 py-1 rounded-full">
                                    {{ __('dashboard.favorited_tag') }}
                                </div>
                            </div>
                            <div class="p-4">
                                <p class="font-bold text-white mb-1">{{ $favPlayer->first_name }} {{ $favPlayer->last_name }}</p>
                                <p class="text-xs text-slate-400 mb-3">{{ $favPlayer->position ?? 'N/A' }} • {{ $favPlayer->current_club ?? __('dashboard.no_club') }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-slate-900/70 border border-slate-700/50 rounded-2xl p-6 text-center">
                ...
                <p class="text-slate-400">{{ __('dashboard.talents_in_favorites') }}</p>
            </div>
            <div class="bg-slate-900/70 border border-slate-700/50 rounded-2xl p-6 text-center">
                <p class="text-4xl font-black text-amber-400 mb-1">{{ $user->recruiter->subscription_tier ?? 'FREE' }}</p>
                <p class="text-slate-400">{{ __('dashboard.current_plan') }}</p>
            </div>
            <div class="bg-gradient-to-br from-indigo-900/50 to-slate-900/70 border border-indigo-500/30 rounded-2xl p-6 flex flex-col justify-center items-center text-center">
                <p class="text-sm text-indigo-300 font-semibold mb-2">{{ __('dashboard.full_access') }}</p>
                <p class="text-slate-400 text-sm">{{ __('dashboard.full_access_desc') }}</p>
            </div>
        </div>

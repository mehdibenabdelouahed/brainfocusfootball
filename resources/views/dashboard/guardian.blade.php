...
@section('title', __('dashboard.title_guardian'))
...
<div class="min-h-screen bg-slate-950 text-white px-4 py-12">
    <div class="max-w-5xl mx-auto">
        ...
        <div class="mb-8">
            <h1 class="text-3xl font-bold mb-2">{{ __('dashboard.role_guardian') }}</h1>
            <p class="text-slate-400">{{ __('dashboard.guardian_tagline') }}</p>
        </div>
        ...
        @if($child && $childUser)
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 mb-8">
                <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-slate-400 text-sm">{{ $child->position ?? __('dashboard.default_child_position') }} • {{ $child->current_club ?? __('dashboard.no_club') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-3 mt-4 md:mt-0">
                        ...
                        <a href="{{ route('profile.show', $childUser->id) }}" class="px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-sm font-semibold transition border border-slate-700">
                            {{ __('dashboard.view_child_public_profile') }}
                        </a>
                    </div>
                </div>
                ...
                <div class="mt-8 pt-8 border-t border-slate-800">
                    ...
                    @else
                        <div class="bg-green-500/10 border border-green-500/30 rounded-xl p-5 mb-6 flex items-start gap-4">
                            ...
                            <div>
                                <h4 class="text-green-400 font-bold mb-1">{{ __('dashboard.consent_granted_title') }}</h4>
                                <p class="text-sm text-slate-300">
                                    {{ __('dashboard.consent_granted_desc', ['date' => $guardianRecord->consent_date ? $guardianRecord->consent_date->format('d/m/Y à H:i') : __('dashboard.recently')]) }}
                                </p>
                            </div>
                        </div>
                    @endif
                ...
                @if($child->matches_played || $child->goals_scored || $child->assists)
                    <div class="mt-8 pt-8 border-t border-slate-800">
                        <h3 class="text-lg font-bold mb-4">{{ __('dashboard.child_sports_overview', ['season' => $child->season ?? __('dashboard.current_season')]) }}</h3>
                        <div class="grid grid-cols-3 gap-4">
                            <div class="bg-slate-800/50 rounded-xl p-4 text-center">
                                <p class="text-2xl font-bold text-amber-400">{{ $child->matches_played ?? 0 }}</p>
                                <p class="text-slate-400 text-xs mt-1">{{ __('dashboard.matches') }}</p>
                            </div>
                            <div class="bg-slate-800/50 rounded-xl p-4 text-center">
                                <p class="text-2xl font-bold text-amber-400">{{ $child->goals_scored ?? 0 }}</p>
                                <p class="text-slate-400 text-xs mt-1">{{ __('dashboard.goals') }}</p>
                            </div>
                            <div class="bg-slate-800/50 rounded-xl p-4 text-center">
                                <p class="text-2xl font-bold text-amber-400">{{ $child->assists ?? 0 }}</p>
                                <p class="text-slate-400 text-xs mt-1">{{ __('dashboard.assists') }}</p>
                            </div>
                        </div>
                    </div>
                @endif
        ...
        @else
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center">
                ...
                <h2 class="text-xl font-bold mb-2">{{ __('dashboard.no_associated_player') }}</h2>
                <p class="text-slate-400">{{ __('dashboard.no_associated_player_desc') }}</p>
            </div>
        @endif

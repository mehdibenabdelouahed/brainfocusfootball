...
<div class="min-h-screen bg-slate-950 text-white px-4 py-12 relative">
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold mb-2">{{ __('profile.edit_header') }}</h1>
            <p class="text-slate-400 text-sm">{{ __('profile.edit_subheader') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('profile.edit') }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        @if(Auth::user()->player?->profile_photo)
                            <div class="mb-2">
                                ...
                                <p class="text-xs text-slate-400 mt-1">{{ __('profile.current_photo') }}</p>
                            </div>
                        @endif
                        ...
                        <p class="mt-1 text-xs text-slate-400">{{ __('profile.photo_hint_edit') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-slate-900/80 border border-slate-700/70 rounded-2xl p-6">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label for="dominant_foot" class="block text-sm font-medium text-slate-300 mb-2">{{ __('profile.preferred_foot') }}</label>
                        <select id="dominant_foot" name="dominant_foot"
                            class="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition">
                            <option value="">{{ __('profile.select') }}</option>
                            <option value="Droit" {{ old('dominant_foot', Auth::user()->player?->dominant_foot) == 'Droit' ? 'selected' : '' }}>{{ __('profile.foot_right') }}</option>
                            <option value="Gauche" {{ old('dominant_foot', Auth::user()->player?->dominant_foot) == 'Gauche' ? 'selected' : '' }}>{{ __('profile.foot_left') }}</option>
                            <option value="Ambidextre" {{ old('dominant_foot', Auth::user()->player?->dominant_foot) == 'Ambidextre' ? 'selected' : '' }}>{{ __('profile.foot_ambidextrous') }}</option>
                        </select>
                    </div>
                    <div>
                        <label for="height_cm" class="block text-sm font-medium text-slate-300 mb-2">{{ __('profile.height') }}</label>
                        <input type="number" id="height_cm" name="height_cm" value="{{ old('height_cm', Auth::user()->player?->height_cm) }}" min="100" max="250"
                            class="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500 transition"
                            placeholder="175">
                    </div>
                    <div>
                        <label for="weight_kg" class="block text-sm font-medium text-slate-300 mb-2">{{ __('profile.weight') }}</label>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        @if(Auth::user()->player?->main_video_file)
                            <div class="mb-2 p-2 bg-slate-800 rounded-lg border border-slate-700 flex items-center gap-2">
                                ...
                                <span class="text-xs text-slate-300">{{ __('profile.current_video') }}</span>
                            </div>
                        @endif
                    ...
                    <div>
                        ...
                        <textarea id="bio" name="bio" rows="4"
                            class="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500 transition"
                            placeholder="{{ __('profile.bio_placeholder') }}">{{ old('bio', Auth::user()->player?->bio) }}</textarea>

...
@section('title', $user->name . ' - ' . __('profile.profile_player_title'))

@section('meta')
    @php $player = $user->player; @endphp
    <meta name="description" content="{{ __('profile.meta_desc', ['name' => $player?->first_name ?? $user->name, 'position' => $player?->position]) }}">
    <meta property="og:title" content="{{ __('profile.og_title', ['name' => $player?->first_name ?? $user->name]) }}">
    <meta property="og:description" content="{{ $player?->position }} • {{ $player?->current_club ?? __('dashboard.no_club') }} • {{ $player?->level }}">
        ...
        <div class="relative mb-8">
            ...
            <div class="relative bg-slate-900/95 border border-slate-700/70 rounded-3xl p-4 sm:p-8 shadow-2xl">
                ...
                <div class="flex flex-col md:flex-row gap-6 mb-8">
                    ...
                    <div class="flex-1">
                                ...
                                <div class="mt-4">
                                    ...
                                    @elseif($plan === 'GRATUIT')
                                        <a href="{{ route('pricing') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-800 border border-slate-700 hover:bg-slate-700 text-slate-400 rounded-xl font-semibold text-sm transition">
                                            ...
                                            {{ __('profile.subscribe_to_contact') }}
                                        </a>
                                    @else
                                        <div class="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-800 border border-slate-700 text-slate-500 rounded-xl font-semibold text-sm cursor-not-allowed" title="{{ __('profile.monthly_quota_reached') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                                            {{ __('profile.monthly_quota_reached') }}
                                        </div>
                                    @endif
                                </div>
                            @endif
                        ...
                        <div class="mt-6 pt-6 border-t border-slate-800/50">
                            <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">{{ __('profile.share_profile') }}</p>
                            <div class="flex flex-wrap gap-2">
                                <button onclick="copyProfileLink()" class="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-800 hover:bg-amber-500 hover:text-slate-950 transition text-xs font-semibold group relative">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 002 2h2a2 2 0 002-2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3"/></svg>
                                    <span id="copy-text">{{ __('profile.copy_link') }}</span>
                                </button>
                                <a href="https://wa.me/?text={{ urlencode(__('profile.share_text', ['name' => $user->name, 'url' => url()->current()])) }}" target="_blank" class="flex items-center gap-2 px-3 py-2 rounded-lg bg-green-600/10 text-green-400 hover:bg-green-600 hover:text-white transition text-xs font-semibold">
                ...
                <div class="grid md:grid-cols-4 gap-4 mb-8">
                    @if($player?->date_of_birth)
                        <div class="bg-slate-800/50 rounded-xl p-4">
                            <p class="text-slate-400 text-xs mb-1">{{ __('profile.age') }}</p>
                            <p class="font-semibold">{{ __('profile.age_years', ['age' => \Carbon\Carbon::parse($player->date_of_birth)->age]) }}</p>
                        </div>
                    @endif
                    @if($player?->height_cm)
                        <div class="bg-slate-800/50 rounded-xl p-4">
                            <p class="text-slate-400 text-xs mb-1">{{ __('profile.height_label') }}</p>
                            <p class="font-semibold">{{ $player->height_cm }} cm</p>
                        </div>
                    @endif
                    @if($player?->weight_kg)
                        <div class="bg-slate-800/50 rounded-xl p-4">
                            <p class="text-slate-400 text-xs mb-1">{{ __('profile.weight_label') }}</p>
                            <p class="font-semibold">{{ $player->weight_kg }} kg</p>
                        </div>
                    @endif
                    @if($player?->dominant_foot)
                        <div class="bg-slate-800/50 rounded-xl p-4">
                            <p class="text-slate-400 text-xs mb-1">{{ __('profile.preferred_foot') }}</p>
                            <p class="font-semibold">{{ $player->dominant_foot === 'Droit' ? __('profile.foot_right') : ($player->dominant_foot === 'Gauche' ? __('profile.foot_left') : __('profile.foot_ambidextrous')) }}</p>
                        </div>
                    @endif
                    @if($player?->jersey_number)
                        <div class="bg-slate-800/50 rounded-xl p-4">
                            <p class="text-slate-400 text-xs mb-1">{{ __('profile.jersey_label') }}</p>
                ...
                @if($player?->season || $player?->matches_played || $player?->goals_scored || $player?->assists)
                    <div class="mb-8">
                        <h2 class="text-xl font-bold text-amber-400 mb-4">{{ __('profile.stats_title', ['season' => $player->season]) }}</h2>
                        <div class="grid grid-cols-3 gap-4">
                            @if($player?->matches_played)
                                <div class="bg-slate-800/50 rounded-xl p-4 text-center">
                                    <p class="text-3xl font-bold text-amber-400">{{ $player->matches_played }}</p>
                                    <p class="text-slate-400 text-sm mt-1">{{ __('dashboard.matches') }}</p>
                                </div>
                            @endif
                            @if($player?->goals_scored)
                                <div class="bg-slate-800/50 rounded-xl p-4 text-center">
                                    <p class="text-3xl font-bold text-amber-400">{{ $player->goals_scored }}</p>
                                    <p class="text-slate-400 text-sm mt-1">{{ __('dashboard.goals') }}</p>
                                </div>
                            @endif
                            @if($player?->assists)
                                <div class="bg-slate-800/50 rounded-xl p-4 text-center">
                                    <p class="text-3xl font-bold text-amber-400">{{ $player->assists }}</p>
                                    <p class="text-slate-400 text-sm mt-1">{{ __('dashboard.assists') }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="mb-8 bg-slate-800/30 rounded-2xl p-4 sm:p-6 border border-slate-700/50 overflow-hidden">
                    <h2 class="text-xl font-bold text-amber-400 mb-6">{{ __('profile.radar_title') }}</h2>
                            ...
                            @foreach(['mental' => 'Mental', 'physique' => 'Physique', 'technique' => 'Technique', 'vitesse' => 'Vitesse', 'vision' => 'Vision', 'social' => 'Social'] as $key => $label)
                                <div class="flex flex-col">
                                    <div class="flex justify-between items-end mb-1">
                                        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('dashboard.radar_labels.' . $label) }}</span>
                ...
                @if($player?->bio)
                    <div class="mb-8">
                        <h2 class="text-xl font-bold text-amber-400 mb-3">{{ __('profile.about') }}</h2>
                ...
                @if($player?->main_video_file || $player?->main_video_url)
                    <div class="mb-8">
                        <h2 class="text-xl font-bold text-amber-400 mb-3">{{ __('profile.main_video') }}</h2>
                ...
                @if($user->goals && count($user->goals) > 0)
                    <div>
                        <h2 class="text-xl font-bold text-amber-400 mb-3">{{ __('profile.objectifs') }}</h2>
                        <ul class="space-y-2">
...
function copyProfileLink() {
    ...
    navigator.clipboard.writeText(url).then(() => {
        textElement.innerText = "{{ __('profile.link_copied') }}";
